...
export default function LedgerPage() {
  ...
  const filteredTransactions = transactions.filter(t => {
    ...
    const matchesSearch = desc.includes(query) || cat.includes(query) || ref.includes(query);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl border border-gray-200 shadow-sm">
          ...
          <h3 className={`text-xl sm:text-2xl font-bold ${totals.balance >= 0 ? 'text-indigo-600' : 'text-red-600'}`}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-100 text-gray-600 text-xs uppercase font-semibold">
              <tr>
                <th className="px-4 sm:px-6 py-3 sm:py-4">তারিখ</th>
                <th className="px-4 sm:px-6 py-3 sm:py-4">ধরণ</th>
                <th className="px-4 sm:px-6 py-3 sm:py-4">বিবরণ</th>
                <th className="px-4 sm:px-6 py-3 sm:py-4">ক্যাটাগরি</th>
                <th className="px-4 sm:px-6 py-3 sm:py-4">রেফারেন্স</th>
                <th className="px-4 sm:px-6 py-3 sm:py-4 text-right">ডেবিট (খরচ)</th>
                <th className="px-4 sm:px-6 py-3 sm:py-4 text-right">ক্রেডিট (আয়)</th>
                <th className="px-4 sm:px-6 py-3 sm:py-4 text-right">ব্যালেন্স</th>
              </tr>
            </thead>
                ...
                filteredTransactions.map((transaction, index) => {
                  ...
                  return (
                    <tr key={transaction.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 sm:px-6 py-3 sm:py-4 text-sm text-red-600 font-medium text-right">
                      ...
                      <td className="px-4 sm:px-6 py-3 sm:py-4 text-sm text-green-600 font-medium text-right">
                        {transaction.type === "income" ? (
                          `৳ ${transaction.amount.toFixed(2)}`
                        ) : (
                          "-"
                      ...
                      <td className={`px-4 sm:px-6 py-3 sm:py-4 text-sm font-medium text-right ${
            ...
            <tfoot className="bg-gray-50 border-t-2 border-gray-300">
              <tr className="font-bold">
                <td colSpan="5" className="px-4 sm:px-6 py-3 sm:py-4 text-right">সর্বমোট:</td>
                <td className="px-4 sm:px-6 py-3 sm:py-4 text-red-600 text-right">
                  ৳ {filteredTransactions
                    .filter(t => t.type === "expense")
                    .reduce((sum, t) => sum + t.amount, 0)
                    .toFixed(2)}
                </td>
                <td className="px-4 sm:px-6 py-3 sm:py-4 text-green-600 text-right">
                  ৳ {filteredTransactions
                    .filter(t => t.type === "income")
                    .reduce((sum, t) => sum + t.amount, 0)
                    .toFixed(2)}
